...
structure diagnostics.  In pilot ensembles the species yields fall
geometrically with constituent number ($\sim 60\times$ per quark from
meson to baryon, flattening with coupling strength), and the fraction of
baryons that form diquark-first is $0.86$--$1.00$ across all expansion
rates and couplings scanned: dynamics \emph{amplifies} the static
estimate.
\end{abstract}
...
\input{sections/s02_classical_model}
\input{sections/s03_methods}
\input{sections/s04_classical_results}
\input{sections/s05_quantum_setup}
\input{sections/s06_initial_states}
\input{sections/s07_hierarchy}
\input{sections/s08_resonances}
\input{sections/s09_protocols}
\input{sections/s10_confrontation}
...
\appendix
\input{sections/app_a_color}
\input{sections/app_b_validation}
\input{sections/app_c_charge_quantization}
\input{sections/app_d_spectral_projectors}
\input{sections/app_e_limits}
\input{sections/app_f_emt}
\input{sections/app_g_pair_spectra}
\input{sections/app_h_flowed_projectors}

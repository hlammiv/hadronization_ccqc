...
\caption{One strong-coupling event ($\alpha_s=0.6$, $H_0=0.05\,c/\fm$,
$N=3024$) at three times, full-box projection.  Members of eventual
clusters are colored and bonded once the cluster has assembled
(all pair separations within $1.2\,\lambda_D(t)$); everything else is
gray.  The plasma at $t=1\fm/c$ carries almost no assembled structure;
mesons condense as bonded pairs through mid-evolution; the final panel
shows the populated state with per-snapshot assembled counts, and
insets zoom a three-quark baryon and a four-body tetraquark.}
\label{fig:render}
\end{figure*}
...
pure photon branching ($P_\gamma=1$; $N=3000$, 16 events/point).
Meson and baryon yields are flat across $\tau_{\rm bound}=5\fm/c$ to
$\infty$: the bound-pair duty cycle inside $d_{\rm ann}$ makes
effective lifetimes long compared to the simulated window.}
\label{fig:scantau}
\end{figure}

At $P_\gamma=1$ every annihilation removes a pair permanently
($14$--$17$ photons escape per event at $N=3000$), yet meson yields
stay flat at $189$--$193$ across the full $\tau_{\rm bound}$ range
(Fig.~\ref{fig:scantau}): the loss of $\sim1\%$ of the quarks is
invisible against ongoing Hubble capture.  The

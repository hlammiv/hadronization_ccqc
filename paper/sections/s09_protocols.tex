...
The systematics budget of an actual campaign then consists of five
measured (not estimated) entries, each tied to a limit of
Appendix~\ref{app:limits}: the $\ell_{\rm cl}$- and $d$-plateaus of
the final counts; the flagged merged-cluster fraction from the
Tier-2 spectrum match; the $t_f$-plateau residual slope per species
(bounded by $\Gamma_s$ for narrow states); the occupancy of Tier-2
spectral-window collisions; and the closure of the Ward-identity sum
rules that normalize $P^\mu_R$.  Every entry feeds the unfolding
matrix of Sec.~\ref{sec:shots-to-ratios}.
